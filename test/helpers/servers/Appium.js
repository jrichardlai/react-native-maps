...
async function startDriver({ caps, appiumServerConfig }) {
  driver = await wd.promiseChainRemote(appiumServerConfig.host, appiumServerConfig.port);
...
class Appium {
  ...
  async start() {
    ...
    this.server = await startAppium({ appiumServerConfig: this.appiumServerConfig });
    this.driver = await startDriver({ caps: this.caps, appiumServerConfig: this.appiumServerConfig });
  }
